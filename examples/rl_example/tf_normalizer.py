...
class TFNormalizer(normalizer.Normalizer):

    def __init__(self, sess, scope, size, init_mean=None, init_std=None, eps=0.02, clip=np.inf):
        self._sess = sess
        self.scope = scope
        super().__init__(size, init_mean, init_std, eps, clip)
    ...
    def _update_resource_tf(self):
        ...
        self.get_sess().run(self._update_op, feed_dict=feed)
        return
